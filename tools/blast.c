/* blast.c -- PKWARE DCL decompression */
/*
...
local int decomp(struct state *s) {
    ...
    if (virgin) {
        construct(&litcode, litlen, (int)sizeof(litlen));
        construct(&lencode, lenlen, (int)sizeof(lenlen));
        construct(&distcode, distlen, (int)sizeof(distlen));
        virgin = 0;
    }
    ...
    do {
        if (bits(s, 1)) {
            ...
            symbol = len == 2 ? 2 : dict;
            dist = (unsigned)decode(s, &distcode) << symbol;
            dist += (unsigned)bits(s, symbol);
            dist++;
...
int blast(
    ...
    if (err != 1 && s.next != 0 && s.outfun(s.outhow, s.out, s.next) && err == 0) {
        err = 1;
    }
    return err;
}
