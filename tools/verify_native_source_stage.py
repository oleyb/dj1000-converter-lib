...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare the native full source stage against the original DLL source-stage output."
    )
    parser.add_argument(
        "--native-binary",
        type=Path,
    ...
    parser.add_argument(
        "--scratch",
        type=Path,
        default=scratch_dir("native_source_stage"),
    )
    parser.add_argument(
        "--modes",
        nargs="*",
    ...
    print(f"verified {verified_cases} full source-stage cases")
    return 0
